...
initialize_watchdog(void)
{
	...
	/* Initialize pgraft consensus if configured */
	if (wd_should_use_pgraft())
	{
		int backend_id;
		char *hostname;
		int port;
		
		/* Safety check: ensure we have at least one backend configured */
		if (pool_config->backend_desc == NULL || pool_config->backend_desc->num_backends == 0)
		{
			ereport(WARNING,
					(errmsg("pgraft consensus: no backends configured, skipping initialization")));
		}
		else
		{
			backend_id = 0;
			hostname = BACKEND_INFO(backend_id).backend_hostname;
			port = BACKEND_INFO(backend_id).backend_port;
			
			/* Safety check: ensure hostname is valid */
			if (hostname == NULL || strlen(hostname) == 0)
			{
				ereport(WARNING,
						(errmsg("pgraft consensus: backend 0 has no hostname, skipping initialization")));
			}
			else
			{
				ereport(LOG,
						(errmsg("watchdog: initializing pgraft consensus mode"),
						 errdetail("connecting to backend %s:%d", hostname, port)));
				
				if (!wd_pgraft_init("pgbalancer-cluster", 
									 pool_config->pgbalancer_node_id,
									 hostname, port))
				{
					ereport(WARNING,
							(errmsg("failed to initialize pgraft consensus"),
							 errhint("ensure pgraft extension is installed on backend nodes")));
				}
				else
				{
					ereport(LOG,
							(errmsg("pgraft consensus initialized successfully"),
							 errdetail("node_id=%d, mode=pgraft", pool_config->pgbalancer_node_id)));
				}
			}
		}
	}
...
watchdog_main(void)
{
	...
	for (;;)
	{
		...
		/*
		 * Check for pgraft leader changes if using pgraft consensus
		 * This runs every iteration to detect leader changes quickly
		 */
		if (wd_should_use_pgraft())
		{
			static int last_detected_leader = -1;
			static int check_counter = 0;
			
			/* Check every 5 iterations (~5 seconds) to avoid excessive queries */
			check_counter++;
			if (check_counter >= 5)
			{
				int current_leader = wd_pgraft_find_leader_backend();
				
				if (current_leader >= 0)
				{
					if (last_detected_leader == -1)
					{
						/* First detection */
						ereport(LOG,
								(errmsg("pgraft consensus: initial leader is backend %d", current_leader)));
						last_detected_leader = current_leader;
					}
					else if (current_leader != last_detected_leader)
					{
						/* Leader changed! */
						ereport(LOG,
								(errmsg("pgraft consensus: leader changed from backend %d to backend %d", 
										last_detected_leader, current_leader)));
						
						/* TODO: Promote the new leader to primary in backend pool */
						/* This would integrate with pgbalancer's existing failover mechanism */
						ereport(NOTICE,
								(errmsg("pgraft consensus: automatic promotion not yet implemented"),
								 errhint("backend %d should be promoted to primary", current_leader)));
...
#define WD_VERIFY_RECEIVED_CONFIG_PARAMETER_VAL_STRING(config_obj, wdNode, parameter) \
do { \
	if (config_obj->parameter && pool_config->parameter && strcmp(config_obj->parameter, pool_config->parameter) != 0) \
	{ \
		ereport(WARNING, \
			(errmsg("configurations value for \"%s\" on node \"%s\" is different", #parameter, wdNode->nodeName), \
				errdetail("\"%s\" on this node is '%s' while on \"%s\" is '%s'", \
